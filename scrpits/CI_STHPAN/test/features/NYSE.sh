...
if [ ! -d "./logs/CI-STHPAN/test/features" ]; then
    mkdir ./logs/CI-STHPAN/test/features
fi
...
python -u run.py \
  ...
  --des 'Exp' \
  --batch_size 1 \
  --itr 1 >logs/CI-STHPAN/test/features/$market_name'_sl'$seq_len'_it'$input_token_len'_pl'$pred_len'_ol'$output_len'_el'$e_layers'_dm'$d_model'_nh'$n_heads'_a'$alpha.log