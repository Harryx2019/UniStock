...
do
    ...
    for i in "${!train_begin_date[@]}"; do
        ...
        --des 'Exp' \
        --batch_size 1 \
        --itr 1 >logs/$model_name/$task_name/$data_path/$pretrain_data/$market_name'_'${test_begin_date[$i]}'_sl'$seq_len'_it'$input_token_len'_pl'$pred_len'_el'$e_layers'_dm'$d_model'_nh'$n_heads'_a'$alpha'_graph'$graph.log
    done
done