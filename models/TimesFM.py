...
class TimesFmHparams:
  ...
  quantiles: None = DEFAULT_QUANTILES # Sequence[float] | None = DEFAULT_QUANTILES
...
def _shift_padded_seq(mask: torch.Tensor, seq: torch.Tensor):# -> torch.Tensor:
  ...
  Args:
    ...
    seq: seq tensor of shape [B, N, P] (position embedding)
...
class TimesFMAttention(nn.Module):
  ...
  def forward(
      ...
      mask: torch.Tensor,
      kv_write_indices = None, #: torch.Tensor | None = None,
      kv_cache = None #: Tuple[torch.Tensor, torch.Tensor] | None = None,
  ) -> torch.Tensor:
...
class StackedDecoder(nn.Module):
  ...
  def forward(
      ...
      paddings: torch.Tensor,
      kv_write_indices=None,#: torch.Tensor | None = None,
      kv_caches=None#: List[Tuple[torch.Tensor, torch.Tensor]] | None = None,
  ) -> torch.Tensor:
...
#   ]:
    ...
    patched_inputs = torch.where(
        torch.abs(patched_pads - 1.0) < self.config.tolerance, # 1代表padding
        torch.tensor(0.0,
    ...
    concat_inputs = torch.cat([patched_inputs, patched_pads], dim=-1) # padding在尾部
  ...
  def forward(
    ...
    input_ts = input_ts.permute(0, 2, 1)
    input_ts = input_ts.reshape(B * C, L)

    if input_padding == None:
      input_padding = torch.zeros(B * C, L).float().to(input_ts.device)
    if freq == None:
      freq = torch.zeros(B * C, 1).int().to(input_ts.device)
    ...
    output_ts = output_ts[:, -1, :, 0] # [B * C, N, output_patch_len, 10]
    output_ts = output_ts.reshape(B, C, -1)
    output_ts = output_ts.permute(0, 2, 1) # [B, output_patch_len, C]
  ...
  def decode(
    ...
    for step_index in range(num_decode_patches):
      ...
      # forward (b, n, self.config.horizon_len, num_outputs)
...
if  __name__ == '__main__':
    ...
    loaded_checkpoint = torch.load('./checkpoints/TimesFM/timesfm-1.0-200m-pytorch.ckpt', weights_only=True)
    ...
    L = 672  # 序列长度
    C = 1
    input_ts = torch.rand(B, L, C).float().to(device)             # 随机张量，取值范围 [0, 1)
    input_padding = None       # 全零张量
    freq = None                # 全零张量

    fprop_outputs = model(input_ts, input_padding, freq)
    # fprop_outputs = fprop_outputs[:, -1, :, 0] # 取最后一个token的mean预测结果
    print(f'fprop_outputs {fprop_outputs.shape}')
